...
def main():
    ...
    while True:
        ...
        if menu == "1":
            ...
            keyword = input("외부에서 검색할 책 제목: ").strip()
            results = search_external_books(keyword)
            ...
            if len(results) == 0:
                ...
                continue
            print_search_results(results)
            number = input_number("등록할 책 번호: ")
            if number is None or number < 1 or number > len(results):
                print("올바른 번호를 입력하세요.")
                continue
            selected_book = results[number - 1]
            add_book(selected_book['title'], selected_book['author'])
            print(f"'{selected_book['title']}' 책이 등록되었습니다.")
